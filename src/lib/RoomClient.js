...
export default class RoomClient {
  ...
  constructor({ roomId, peerName, displayName, device, useSimulcast, forceTcp, spy, forceH264 }) {
    logger.debug(
  ...
  changeDisplayName(displayName) {
    ...
    return this._protoo
      .send("change-display-name", { displayName })
      .then(() => {
  ...
  changeWebcam() {
    ...
    return Promise.resolve()
      ...
      .then(newTrack => {
        store.dispatch(stateActions.setProducerTrack(this._webcamProducer.id, newTrack))
  ...
  disableAudioOnly() {
    ...
    return Promise.resolve()
      .then(() => {
        ...
        if (!this._webcamProducer && this._room.canSend("video")) return this.enableWebcam()
      })
      .then(() => {
        for (const peer of this._room.peers) {
          for (const consumer of peer.consumers) {
  ...
  changeConsumerPreferredProfile(consumerId, profile) {
    ...
    return this._protoo
      .send("change-consumer-preferred-profile", {
        consumerId,
        profile
      })
      .then(() => {
        ...
        store.dispatch(stateActions.setConsumerPreferredProfile(consumerId, profile))
      })
      .catch(error => {
  ...
  requestConsumerKeyFrame(consumerId) {
    ...
    return this._protoo
      .send("request-consumer-keyframe", { consumerId })
      .then(() => {
        store.dispatch(
          requestActions.notify({
  ...
  _join({ displayName, device }) {
    ...
    this._protoo.on("disconnected", () => {
      ...
      // Leave Room.
      try {
        this._room.remoteClose({ cause: "protoo disconnected" })
      } catch (error) {}
    ...
    this._protoo.on("notification", notification => {
      ...
      switch (notification.method) {
        ...
        case "active-speaker": {
          ...
          store.dispatch(stateActions.setRoomActiveSpeaker(peerName))
        ...
        case "display-name-changed": {
          ...
          store.dispatch(stateActions.setPeerDisplayName(displayName, peerName))
  ...
  _joinRoom({ displayName, device }) {
    ...
    this._room
      .join(this._peerName, { displayName, device })
      .then(() => {
  ...
  _handleConsumer(consumer) {
    ...
    consumer.on("close", originator => {
      ...
      store.dispatch(stateActions.removeConsumer(consumer.id, consumer.peer.name))
    })
    ...
    if (consumer.supported) {
      ...
      if (consumer.kind === "video" && store.getState().me.audioOnly) consumer.pause("audio-only-mode")

      consumer
        .receive(this._recvTransport)
        .then(track => {
